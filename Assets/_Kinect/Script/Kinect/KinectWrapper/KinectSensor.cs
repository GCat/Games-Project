...
public class KinectSensor : MonoBehaviour, KinectInterface {
    ...
    private NuiImageViewArea viewArea;
    ...
    bool KinectInterface.pollDepth()
    {
        if (!updatedDepth)
        {
            ...
            if (hr == 0) {
                ...
                viewArea = imageFrame.ViewArea_NotUsed;
                INuiFrameTexture frameTexture = (INuiFrameTexture)Marshal.GetObjectForIUnknown(imageFrame.pFrameTexture);
    ...
    short[] KinectInterface.getMappedDepth() {
        ...
        for (int j = 0; j < 320; j++)
        {
            ...
            long lDepthX = j;
            long lDepthY = i;
            ushort depthValue = (ushort)(depthPlayerData[i*320 + j]);
            ...
            long pColorY = 0;
                ...
                if (depthValue != 0)
                {
                    result = Kinect.NativeMethods.NuiImageGetColorPixelCoordinatesFromDepthPixelAtResolution(NuiImageResolution.resolution640x480, NuiImageResolution.resolution320x240, ref viewArea, lDepthX, lDepthY, depthValue, ref pColorX, ref pColorY);

                    if (pColorY < 480 && pColorX < 640 && result != 0)
                    {
                        mappedDepth[pColorY * 640 + pColorX] = (short)(depthValue >> 3);
                        count++;
                    }
	...
	private short[] mapDepth(NuiLockedRect depthIn, uint inLength, uint outLength)
    {
        short[] newDepth = new short[outLength];
        return newDepth;
    }
